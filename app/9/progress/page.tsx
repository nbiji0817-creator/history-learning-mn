...
export default function Grade9ProgressPage() {
  const [progress, setProgress] = useState<ProgressData>({});
  const [studentName, setStudentName] = useState("");

  useEffect(() => {
    const savedProgress = localStorage.getItem("history9-progress");
    const savedName = localStorage.getItem("history9-student");
  ...
  function clearProgress() {
    const confirmed = window.confirm(
      "9-р ангийн бүх ахиц, оноог устгах уу?"
    );

    if (!confirmed) return;

    localStorage.removeItem("history9-progress");
    setProgress({});
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        {/* STUDENT */}
        <div className="rounded-3xl border border-cyan-400/20 bg-gradient-to-r from-cyan-950/50 to-violet-950/50 p-7">
          ...
          <p className="mt-2 text-slate-400">
            9-р ангийн орчин үеийн Монголын түүхийн ахиц
          </p>
        </div>
